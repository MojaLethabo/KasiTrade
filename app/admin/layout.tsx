...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentLabel = adminNav.find(n => pathname === n.href || (n.href !== "/admin" && pathname.startsWith(n.href)))?.label || "";
    ...
    <>
      ...
      <div style={{ padding: "12px 8px", borderTop: "1px solid var(--border)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "9px", padding: "8px 12px", marginBottom: "6px" }}>
          <div style={{ width: "28px", height: "28px", background: "var(--bg-4)", border: "1px solid var(--border-2)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <Shield size={12} color="var(--text-2)" />
          </div>
          <div>
            <div style={{ fontSize: "12px", fontWeight: "600", color: "var(--text)" }}>Admin</div>
            <div style={{ fontSize: "10px", color: "var(--text-3)" }}>Centre Administrator</div>
          </div>
        </div>
    ...
    <div className="app-shell">
      ...
      <header className="topbar">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <span style={{ fontSize: "15px", fontWeight: "700", letterSpacing: "-0.02em" }}>KasiTrade Admin</span>
        </div>
        {currentLabel && <span style={{ fontSize: "12px", color: "var(--text-3)", marginLeft: "auto" }}>{currentLabel}</span>}
      </header>
      ...
      <div className="shell-body">
        ...
        <aside className={`sidebar${open ? " open" : ""}`}>
          <div style={{ display: "flex", justifyContent: "flex-end", padding: "10px 12px 0", flexShrink: 0 }}>
            <button onClick={() => setOpen(false)} className="hamburger" aria-label="Close"><X size={18} /></button>
          </div>
          {sidebarContent}
        </aside>
