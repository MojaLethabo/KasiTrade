      ...
      <div className="page-wrap">
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "12px", flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ position: "relative", flex: "1", minWidth: "200px", maxWidth: "320px" }}>
            <Search size={13} style={{ position: "absolute", left: "11px", top: "50%", transform: "translateY(-50%)", color: "var(--ink-3)", pointerEvents: "none" }} />
          ...
          <button
            ...
            style={{ borderColor: activeFilters > 0 ? "var(--border-2)" : undefined, color: activeFilters > 0 ? "var(--ink)" : undefined }}
          >
        ...
        {processed.length === 0 ? (
          <div className="card" style={{ padding: "40px", textAlign: "center" }}>
            <p style={{ color: "var(--ink-3)", marginBottom: "12px" }}>No entrepreneurs match your filters.</p>
                ...
                <div key={e.id} className="card" style={{ overflow: "hidden" }}>
                  ...
                  <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)", display: "flex", flexWrap: "wrap", gap: "10px", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px", flex: 1, minWidth: "200px" }}>
                      <div style={{ width: "36px", height: "36px", background: "var(--surface-3)", border: "1px solid var(--border-2)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: "700", flexShrink: 0 }}>
                        {e.name[0]}
                      </div>
                  ...
                  <div style={{ overflowX: "auto" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(8, minmax(90px, 1fr))", background: "var(--surface-2)", minWidth: "600px" }}>
                      {[
                        ...
                        <div key={label} style={{ padding: "10px 14px", borderRight: i < 7 ? "1px solid var(--border)" : "none" }}>
                          ...
                          <div style={{ fontSize: "12px", fontWeight: bold ? "700" : "500", color: bold ? "var(--ink)" : "var(--ink-2)" }}>{value}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Progress bars */}
                  <div style={{ padding: "12px 18px", background: "var(--surface)", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "14px" }}>
                    {[
                      ...
                      <div key={bar.label}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                          <span style={{ fontSize: "10px", color: "var(--ink-3)" }}>{bar.label}</span>
                          <span style={{ fontSize: "10px", fontWeight: "600", color: "var(--ink-2)" }}>{Math.round(bar.pct)}%</span>
                        </div>
