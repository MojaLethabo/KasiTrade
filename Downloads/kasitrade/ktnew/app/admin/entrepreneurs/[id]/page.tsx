  ...
  if (!user || user.role !== "entrepreneur") {
    return (
      <main className="main-content">
        <p style={{ color: "var(--ink-3)", marginBottom: "12px" }}>Entrepreneur not found.</p>
    ...
    <main className="main-content">
      <div className="page-wrap">
        ...
        <div style={{ marginBottom: "24px" }}>
          <Link href="/admin/entrepreneurs" style={{ display: "inline-flex", alignItems: "center", gap: "6px", color: "var(--ink-3)", textDecoration: "none", fontSize: "12px", marginBottom: "14px" }}>
            <ArrowLeft size={13} /> Back to Entrepreneurs
          </Link>
          <div style={{ display: "flex", alignItems: "flex-start", gap: "14px" }}>
            <div style={{ width: "46px", height: "46px", background: "var(--surface-3)", border: "1px solid var(--border-2)", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", fontWeight: "700", flexShrink: 0 }}>
              {user.name[0]}
            </div>
            <div>
              <h1 style={{ marginBottom: "3px" }}>{user.name}</h1>
              <p style={{ color: "var(--ink-3)", fontSize: "13px" }}>{b.name} · {user.email}</p>
            </div>
          </div>
        </div>
          ...
          {[
            ...
            <div key={bar.label}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "5px" }}>
                <span style={{ fontSize: "11px", color: "var(--ink-3)" }}>{bar.label}</span>
                <span style={{ fontSize: "11px", fontWeight: "600", color: "var(--ink-2)" }}>{Math.round(bar.pct)}%</span>
              </div>
              <div style={{ height: "5px", background: "var(--surface-3)", borderRadius: "3px", overflow: "hidden" }}>
                <div style={{
                  height: "100%", width: `${bar.pct}%`, borderRadius: "3px",
                  background: bar.pct >= 70 ? "var(--ink-2)" : bar.pct >= 40 ? "rgba(255,255,255,0.38)" : "rgba(255,90,90,0.55)",
                }} />
              </div>
            </div>
          ))}
        ...
        <div className="grid-2" style={{ marginBottom: "16px" }}>
          ...
          <div className="card" style={{ padding: "18px 20px" }}>
            ...
            <div style={{ display: "grid", gap: "10px" }}>
              {[
                ...
                <div key={label} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <div style={{ width: "26px", height: "26px", background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "4px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Icon size={11} strokeWidth={1.5} color="var(--ink-3)" />
                  </div>
                  <div>
            ...
            {b.description && <p style={{ marginTop: "12px", paddingTop: "12px", borderTop: "1px solid var(--border)", fontSize: "12px", color: "var(--ink-3)", lineHeight: "1.6" }}>{b.description}</p>}
          </div>
          ...
          <div className="card" style={{ padding: "18px 20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px", marginBottom: "14px" }}>
              {[
                ...
                <div key={label} style={{ padding: "10px", background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "4px", textAlign: "center" }}>
                  <div style={{ fontSize: "18px", fontWeight: "700", color: "var(--ink)", marginBottom: "2px" }}>{value}</div>
                  <div style={{ fontSize: "10px", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
              {user.loginDates.map((d, i) => (
                <span key={i} style={{ fontSize: "10px", color: "var(--ink-3)", background: "var(--surface-2)", border: "1px solid var(--border)", padding: "3px 7px", borderRadius: "3px", fontFamily: "monospace" }}>{d}</span>
              ))}
            </div>
          </div>
        </div>
              ...
              <table className="tbl" style={{ minWidth: "480px" }}>
                ...
                <tbody>
                  {data.invoices.map(inv => (
                    <tr key={inv.id}>
                      <td style={{ fontFamily: "monospace", fontSize: "11px", color: "var(--ink-3)" }}>{inv.id.toUpperCase()}</td>
                      <td style={{ fontWeight: "500" }}>{inv.customerName}</td>
                      <td style={{ color: "var(--ink-3)", fontSize: "12px" }}>{inv.date}</td>
                      <td style={{ textAlign: "right", fontWeight: "600" }}>{fmt(inv.total)}</td>
                      <td style={{ textAlign: "right", color: "var(--ink-2)" }}>{fmt(inv.paidAmount)}</td>
              ...
              <table className="tbl" style={{ minWidth: "420px" }}>
                ...
                <tbody>
                  {data.expenses.map(exp => (
                    <tr key={exp.id}>
                      <td style={{ color: "var(--ink-3)", fontSize: "12px" }}>{exp.date}</td>
                ...
                <tbody>
                  {data.customers.map(c => (
                    <tr key={c.id}>
                      ...
                      <td style={{ color: "var(--ink-3)", fontSize: "12px" }}>{c.email}</td>
                      <td style={{ fontSize: "12px" }}>{c.phone}</td>
                      <td style={{ color: "var(--ink-3)", fontSize: "12px" }}>{c.createdAt}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
