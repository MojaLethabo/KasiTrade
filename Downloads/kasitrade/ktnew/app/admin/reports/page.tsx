...
import {
  users, invoices, receipts, expenses,
  getTotalIncome, getTotalExpenses, getPlatformMonthlyTrend,
} from "@/lib/data";
...
export default function ReportsPage() {
  ...
  const chartData = trend.map(t => ({
    label: t.month.split(" ")[0], // "Jan", "Feb" etc
    income:   t.income,
    expenses: t.expenses,
  }));
  ...
  function exportEntrepreneurs() {
    downloadCSV(filtered.map(e => ({
      ...
      Income: getTotalIncome(e.id), Expenses: getTotalExpenses(e.id),
      Profit: getTotalIncome(e.id)-getTotalExpenses(e.id),
      Logins: e.loginDates.length, Activity: getActivity(e.loginDates.length),
      Joined: e.createdAt, LastLogin: e.lastLogin,
    ...
    <main className="main-content">
      <div className="page-wrap">
        <div style={{ marginBottom: "24px" }}>
          ...
          <p style={{ color: "var(--ink-3)", fontSize: "13px" }}>Platform analytics, income trends, and CSV export.</p>
        </div>
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "20px", flexWrap: "wrap", alignItems: "center" }}>
          ...
          <select className="input" style={{ width: "170px" }} value={sectorFilter} onChange={e => setSectorFilter(e.target.value)}>
        ...
        <div className="card" style={{ overflow: "hidden", marginBottom: "20px" }}>
          <div style={{ padding: "16px 18px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "10px" }}>
            <div>
              <h3 style={{ marginBottom: "3px" }}>Monthly Income Trend</h3>
              <p style={{ fontSize: "11px", color: "var(--ink-3)" }}>Platform-wide income vs expenses over time</p>
            </div>
            <button className="btn btn-ghost btn-sm" onClick={exportTrendCsv}><Download size={12} /> Export CSV</button>
          </div>
            ...
            <table className="tbl" style={{ minWidth: "600px" }}>
                ...
                {trend.map((t, i) => {
                  ...
                  return (
                    <tr key={t.month}>
                      ...
                      <td style={{ textAlign:"right", fontWeight:"600" }}>{t.income > 0 ? fmt(t.income) : <span style={{ color:"var(--ink-3)" }}>—</span>}</td>
                      <td style={{ textAlign:"right", color:"var(--ink-2)" }}>{t.expenses > 0 ? fmt(t.expenses) : <span style={{ color:"var(--ink-3)" }}>—</span>}</td>
                      <td style={{ textAlign:"right", fontWeight:"600", color: profit < 0 ? "#ff8080" : undefined }}>{(t.income>0||t.expenses>0) ? fmt(profit) : <span style={{ color:"var(--ink-3)" }}>—</span>}</td>
                      <td style={{ textAlign:"right" }}>
                        {t.income > 0
                          ? <span className={`badge ${margin>=50?"badge-green":margin>=20?"badge-yellow":"badge-red"}`}>{margin}%</span>
                          : <span style={{ color:"var(--ink-3)",fontSize:"12px" }}>—</span>}
                      </td>
                      <td style={{ textAlign:"right" }}>{t.invoicesIssued || <span style={{ color:"var(--ink-3)" }}>0</span>}</td>
                      <td style={{ textAlign:"right" }}>{t.invoicesPaid   || <span style={{ color:"var(--ink-3)" }}>0</span>}</td>
                      <td style={{ textAlign:"right" }}>{t.newCustomers   || <span style={{ color:"var(--ink-3)" }}>0</span>}</td>
                      <td style={{ textAlign:"center" }}>
                        {i===0 ? <Minus size={12} color="var(--ink-3)" />
                          : change>0 ? <TrendingUp size={12} color="var(--ink-2)" />
                          : change<0 ? <TrendingDown size={12} color="#ff8080" />
                          : <Minus size={12} color="var(--ink-3)" />}
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot>
                <tr style={{ borderTop:"2px solid var(--border-2)" }}>
                  <td style={{ fontWeight:"700", padding:"11px 14px" }}>Totals</td>
                  <td style={{ textAlign:"right", fontWeight:"700", padding:"11px 14px" }}>{fmt(trend.reduce((s,t)=>s+t.income,0))}</td>
                  <td style={{ textAlign:"right", fontWeight:"600", padding:"11px 14px", color:"var(--ink-2)" }}>{fmt(trend.reduce((s,t)=>s+t.expenses,0))}</td>
                  <td style={{ textAlign:"right", fontWeight:"700", padding:"11px 14px" }}>{fmt(trend.reduce((s,t)=>s+t.income-t.expenses,0))}</td>
                  <td colSpan={2} style={{ padding:"11px 14px" }}></td>
                  <td style={{ textAlign:"right", fontWeight:"600", padding:"11px 14px" }}>{trend.reduce((s,t)=>s+t.invoicesPaid,0)}</td>
                  <td style={{ textAlign:"right", fontWeight:"600", padding:"11px 14px" }}>{trend.reduce((s,t)=>s+t.newCustomers,0)}</td>
                  <td></td>
                </tr>
              </tfoot>
            </table>
            ...
            {Object.entries(bySector).map(([sec, d]) => (
              <div key={sec} style={{ marginBottom: "10px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"4px" }}>
                  ...
                  <span style={{ fontSize:"11px", color:"var(--ink-3)" }}>{d.count} · {fmt(d.income)}</span>
                </div>
                <div style={{ height:"3px", background:"var(--surface-3)", borderRadius:"2px", overflow:"hidden" }}>
                  <div style={{ height:"100%", width:`${(d.count/entrepreneurs.length)*100}%`, background:"var(--brand)", borderRadius:"2px" }} />
                </div>
              </div>
            ))}
          </div>
            ...
            {(["active","moderate","low"] as const).map(level => {
              ...
              return (
                <div key={level} style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"10px" }}>
                  ...
                  <span style={{ fontSize:"11px", color:"var(--ink-3)", width:"18px", textAlign:"right" }}>{count}</span>
                </div>
              );
            })}
            <div className="divider" style={{ margin:"12px 0" }} />
            <div className="section-label" style={{ marginBottom:"10px" }}>Business Stages</div>
              ...
              return (
                <div key={status} style={{ marginBottom:"12px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"4px" }}>
                    ...
                    <div><span style={{ fontSize:"12px", fontWeight:"600" }}>{d.count}</span><span style={{ fontSize:"11px", color:"var(--ink-3)", marginLeft:"6px" }}>{fmt(d.value)}</span></div>
                  </div>
          ...
          <div style={{ display:"grid", gap:"8px" }}>
            {Object.entries(byCat).sort((a,b)=>b[1]-a[1]).map(([cat, total]) => (
              <div key={cat} style={{ display:"grid", gridTemplateColumns:"110px 1fr 90px", gap:"10px", alignItems:"center" }}>
                ...
                <div style={{ height:"4px", background:"var(--surface-3)", borderRadius:"2px", overflow:"hidden" }}>
                  <div style={{ height:"100%", width:`${(total/maxCat)*100}%`, background:"var(--border-2)", borderRadius:"2px" }} />
                </div>
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <div className="card" style={{ overflow:"hidden" }}>
            <div className="tbl-wrap">
              <table className="tbl" style={{ minWidth:"600px" }}>
                <thead>
                  <tr><th>Entrepreneur</th><th>Sector</th><th>Stage</th><th style={{ textAlign:"right" }}>Income</th><th style={{ textAlign:"right" }}>Expenses</th><th style={{ textAlign:"right" }}>Profit</th><th style={{ textAlign:"center" }}>Inv.</th><th style={{ textAlign:"center" }}>Paid%</th><th style={{ textAlign:"center" }}>Logins</th><th>Activity</th></tr>
                </thead>
                <tbody>
                  {filtered.map(e => {
                    ...
                    return (
                      <tr key={e.id}>
                        ...
                        <td style={{ textAlign:"right", color:"var(--ink-2)" }}>{fmt(exp)}</td>
        ...
        <div className="card" style={{ padding: "18px 20px" }}>
          ...
          <p style={{ color: "var(--ink-3)", fontSize: "13px", marginBottom: "16px" }}>Download CSV files. Exports respect current filters.</p>
          <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
            ...
            <button className="btn btn-secondary btn-sm" onClick={exportInvoicesCsv}><Download size={12} /> Invoices ({filteredInvoices.length})</button>
            <button className="btn btn-secondary btn-sm" onClick={exportExpensesCsv}><Download size={12} /> Expenses ({filteredExpenses.length})</button>
            <button className="btn btn-secondary btn-sm" onClick={exportTrendCsv}><Download size={12} /> Monthly Trend</button>
            <button className="btn btn-ghost btn-sm" onClick={() => { exportEntrepreneurs(); setTimeout(exportInvoicesCsv,400); setTimeout(exportExpensesCsv,800); setTimeout(exportTrendCsv,1200); }}>
              Export All (4 files)
            </button>
          </div>
        </div>
