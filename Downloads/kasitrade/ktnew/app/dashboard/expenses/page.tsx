      ...
      <div className="page-wrap">
        <div className="page-header">
          <div><h1 style={{ marginBottom:"4px" }}>Expenses</h1>
            <p style={{ color:"var(--ink-3)", fontSize:"13px" }}>{expenses.length} expenses · Total: <strong style={{ color:"var(--ink)" }}>{fmt(total)}</strong></p>
          </div>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:"1px", background:"var(--border)", borderRadius:"6px", overflow:"hidden", marginBottom:"20px" }}>
          {Object.entries(byCategory).map(([cat,amt]) => (
            <div key={cat} style={{ background:"var(--surface)", padding:"14px 18px", minWidth:"130px", flex:"1" }}>
              <div className="section-label" style={{ marginBottom:"5px" }}>{cat}</div>
              <div style={{ fontSize:"16px", fontWeight:"700", color:"var(--ink)" }}>{fmt(amt)}</div>
            </div>
          ))}
        </div>
            ...
            <table className="tbl" style={{ minWidth:"400px" }}>
              ...
              <tbody>
                {[...expenses].reverse().map(exp => (
                  <tr key={exp.id}>
                    <td style={{ color:"var(--ink-3)", fontSize:"12px", whiteSpace:"nowrap" }}>{exp.date}</td>
