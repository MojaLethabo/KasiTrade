  ...
  return (
    <div className="card" style={{ overflow: "hidden" }}>
      <div style={{ padding: "16px 18px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button onClick={onCancel} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)" }}><X size={18} /></button>
      </div>
      <div style={{ padding: "18px" }}>
          ...
          <div key={line.id} className="line-item-row">
            <div style={{ position: "relative", gridColumn: "1 / -1" }}>
              ...
              <button onClick={() => setPicker(picker === line.id ? null : line.id)}
                style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)" }}>
                <ChevronDown size={14} />
              </button>
              {picker === line.id && (
                <div style={{ position: "absolute", top: "calc(100%+3px)", left: 0, right: 0, background: "var(--surface-2)", border: "1px solid var(--border-2)", borderRadius: "6px", zIndex: 30, maxHeight: "200px", overflowY: "auto", boxShadow: "0 8px 24px rgba(0,0,0,0.5)" }}>
                  {products.length === 0 ? <div style={{ padding: "12px", fontSize: "12px", color: "var(--ink-3)" }}>No products saved.</div>
                    : products.map(p => (
                      <div key={p.id} onClick={() => pickProd(line.id, p)}
                        ...
                        <span style={{ fontSize: "12px", color: "var(--ink-3)" }}>R {p.price}/{p.unit}</span>
                      </div>
                    ))}
                </div>
              )}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <button onClick={() => setLines(ls => ls.filter(l => l.id !== line.id))} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)", padding: "4px" }}><Trash2 size={13} /></button>
            </div>
          </div>
        ))}
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", margin: "16px 0" }}>
          <div style={{ background: "var(--surface-2)", border: "1px solid var(--border-2)", padding: "12px 18px", borderRadius: "6px", display: "flex", gap: "16px", alignItems: "center" }}>
            <span className="section-label">Total</span>
            <span style={{ fontSize: "20px", fontWeight: "700", color: "var(--ink)" }}>{fmt(total)}</span>
          </div>
        </div>
      ...
      <div className="page-wrap">
        <div className="page-header">
          <div>
            ...
            <p style={{ color: "var(--ink-3)", fontSize: "13px" }}>{quotes.length} quotation{quotes.length !== 1 ? "s" : ""}</p>
          </div>
          {!showNew && !editing && (
        ...
        {quotes.length === 0 && !showNew ? (
          <div className="card" style={{ padding: "40px", textAlign: "center" }}>
            <p style={{ color: "var(--ink-3)", marginBottom: "14px" }}>No quotations yet.</p>
          ...
          <div style={{ display: "grid", gap: "10px" }}>
            {quotes.map(q => (
              <div key={q.id} className="card" style={{ overflow: "hidden" }}>
                <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)" }}>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                      ...
                      <div style={{ fontSize: "11px", color: "var(--ink-3)" }}>{q.id.toUpperCase()} · {q.date} → {q.validUntil}</div>
                    </div>
                    ...
                    <thead>
                      <tr>
                        {["Item", "Qty", "Unit", "Subtotal"].map((h, i) => (
                          <th key={h} style={{ textAlign: i > 0 ? "right" : "left", padding: "8px 18px", fontSize: "10px", fontWeight: "700", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.08em", borderBottom: "1px solid var(--border)", background: "var(--surface-2)" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {q.items.map((it, i) => (
                        <tr key={i} style={{ borderBottom: i < q.items.length - 1 ? "1px solid var(--border)" : "none" }}>
                          ...
                          <td style={{ padding: "10px 18px", textAlign: "right", color: "var(--ink-2)" }}>{it.qty}</td>
                          <td style={{ padding: "10px 18px", textAlign: "right", color: "var(--ink-2)" }}>{fmt(it.unitPrice)}</td>
                ...
                {q.notes && <div style={{ padding: "10px 18px", background: "var(--surface-2)", borderTop: "1px solid var(--border)", fontSize: "12px", color: "var(--ink-3)", fontStyle: "italic" }}>{q.notes}</div>}
              </div>
            ))}
          </div>
        )}
        ...
        {printId && printQ && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 50, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "16px", overflowY: "auto" }}>
            <div style={{ background: "var(--surface)", borderRadius: "8px", width: "100%", maxWidth: "760px", marginTop: "8px" }}>
