    ...
    <main className="main-content">
      <div className="page-wrap">
        ...
        <div style={{ marginBottom: "28px" }}>
          ...
          <p style={{ color: "var(--ink-3)", fontSize: "13px" }}>{user.business?.name} · {user.business?.location}</p>
        </div>
        ...
        <div className="grid-2" style={{ marginBottom: "16px" }}>
          ...
          <div className="card" style={{ overflow: "hidden" }}>
            ...
            {data.invoices.length === 0 ? (
              <div style={{ padding: "20px", fontSize: "12px", color: "var(--ink-3)" }}>No invoices yet.</div>
              ...
              <div key={inv.id} style={{ padding: "11px 18px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "8px" }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: "11px", color: "var(--ink-3)" }}>{inv.date}</div>
                </div>
          ...
          <div className="card" style={{ overflow: "hidden" }}>
            <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <Link href="/dashboard/expenses" style={{ fontSize: "11px", color: "var(--ink-3)", textDecoration: "none", display: "flex", alignItems: "center", gap: "3px" }}>
                All <ArrowUpRight size={11} />
              </Link>
            </div>
            {data.expenses.length === 0 ? (
              <div style={{ padding: "20px", fontSize: "12px", color: "var(--ink-3)" }}>No expenses yet.</div>
              ...
              <div key={exp.id} style={{ padding: "11px 18px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "8px" }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: "11px", color: "var(--ink-3)" }}>{exp.category} · {exp.date}</div>
                </div>
                <div style={{ fontSize: "13px", fontWeight: "600", color: "var(--ink-2)", flexShrink: 0 }}>–{fmt(exp.amount)}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1px", background: "var(--border)", borderRadius: "6px", overflow: "hidden" }}>
          {[
            ...
            <div key={label} style={{ background: "var(--surface)", padding: "16px 12px", textAlign: "center" }}>
              <Icon size={15} strokeWidth={1.5} color="var(--ink-3)" style={{ marginBottom: "6px" }} />
              <div style={{ fontSize: "20px", fontWeight: "700", color: "var(--ink)", marginBottom: "2px" }}>{value}</div>
              <div style={{ fontSize: "10px", color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{label}</div>
            </div>
          ))}
        </div>
      </div>
